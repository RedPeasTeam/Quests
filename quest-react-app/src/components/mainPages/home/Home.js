        ...
        return (
            <div>
                <div className="section__gallery" style={{backgroundImage: `url(${img})`}}>
                    ...
                    <p>
                        Выбирайте лучшие квесты Минска по независимым рейтингам и отзывам игроков. Приключенческий
                        квест, детективное расследование или страшный перформанс с актёрами — выбор за вами!
                    </p>
                </div>
                <div className='quest-gallery-nav'>
                    {this.state.main.map((elem, index) => {
                        return <div className='quest-item-wrapper' key={index}>
                            <QuestItem imgSrc={elem.image} questName={elem.name} difficulty={elem.genre} amount={`10-${elem.id}`}/>
                        </div>
                    })}
                </div>
            </div>
